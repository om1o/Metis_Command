...
async def _auth_middleware(request, call_next):
    ...
    # Only allow query param token for export download links
    if not token and request.url.path.endswith("/export"):
...
_JOB_KEYWORDS = {"everyday", "daily", "weekly", "monthly", "recurring", "schedule", "every morning", "every night", "every week", "every month", "every day", "each day", "each week", "each month"}

def _is_job(msg: str) -> bool:
    lower = msg.lower()
...
async def chat(req: ChatRequest, request: Request) -> StreamingResponse:
    ...
    def sse() -> Any:
        full_answer = ""
        if _is_job(req.message):
            from memory import create_job
            job = create_job(
                title=req.message[:60],
                description=req.message,
                schedule=_extract_schedule(req.message),
            )
            yield f"event: job_created\ndata: {json.dumps({'job_id': job['id'], 'title': job['title']})}\n\n"
        if req.role == "manager":
            ...
            # Load session context for non-manager roles too
        ...
        if full_answer:
            ...
            # On the first turn, generate a smart 4-6 word title via the
            # Manager model; fall back to the auto-truncated title otherwise.
            try:
                from memory import message_count, rename_session
                if message_count(req.session_id, user_id=user_id) == 2:
                    from brain_engine import chat_by_role
                    raw = chat_by_role(
                        "manager",
                        [{"role": "user", "content": (
                            "Reply with ONLY a short title (4-6 words, no quotes, "
                            "no punctuation) that describes this conversation:\n"
                            f"User: {req.message[:200]}\n"
                            f"Assistant: {full_answer[:200]}"
                        )}],
                        temperature=0.3,
                    )
                    smart = raw.strip().strip("\"'").strip()[:80]
                    if smart:
                        rename_session(req.session_id, smart, user_id=user_id)
                        session_title = smart
            except Exception as e:
                print(f"[api_bridge] smart title failed (non-fatal): {e}")

            # Fallback: use the auto-truncated title set by save_message.
...
def conversation_search(
    ...
    """Full-text search across stored conversation messages.

    Returns ``[{session_id, title, snippet, created_at, role}, ...]``,
    one entry per matching session, ordered by recency.
    """
    user_id = _user_id_from_request(request)
    from memory import search_messages
    try:
        return search_messages(q, user_id=user_id, limit=limit)
    except Exception as e:
        print(f"[api_bridge] /search failed: {e}")
        return []
...
def sessions_export(
    ...
    messages = []
    try:
        messages = list(load_session(session_id, limit=limit, user_id=user_id) or [])
    except Exception as e:
...
@app.get("/jobs")
def jobs_list(status: str | None = None) -> list[dict]:
    from memory import list_jobs
    return list_jobs(status=status)


@app.post("/jobs")
def jobs_create(req: JobCreateRequest) -> dict:
    from memory import create_job
    return create_job(req.title, req.description, req.schedule, req.agent_team)


@app.delete("/jobs/{job_id}")
def jobs_delete(job_id: str) -> dict:
    from memory import delete_job
    delete_job(job_id)
...
def jobs_update_status(job_id: str, req: JobStatusRequest) -> dict:
    from memory import update_job_status
    update_job_status(job_id, req.status)
    return {"ok": True, "id": job_id, "status": req.status}
...
def marketplace_list() -> list[dict]:
    ...
    for p in plugins:
        slug = p.get("slug", "")
        p["installed"] = bool(slug) and (plugins_dir / f"{slug}.py").exists()
    return plugins
...
def marketplace_uninstall(req: MarketplaceInstallRequest) -> dict:
    from pathlib import Path
    slug = req.slug
    dest = Path("plugins") / f"{slug}.py"
    if dest.exists():
        dest.unlink()
        return {"ok": True, "slug": slug}
