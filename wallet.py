...
def handle_stripe_webhook(body: bytes | str, signature: str = "") -> dict:
    """
    Process a Stripe webhook delivery and top up the wallet on
    `checkout.session.completed`.

    Verification is gated by STRIPE_WEBHOOK_SECRET. If the secret is unset
    we still parse the JSON body but log a warning — Stripe webhooks should
    always be signed in production.

    Returns a dict {ok, event_id, type, ...} the API can return as JSON.
    """
    ...
    if secret:
        try:
            ...
            event = stripe.Webhook.construct_event(body_bytes, signature, secret)
            # Stripe SDK returns a dict-like Event; normalize to dict.
            event_payload = dict(event)
        except Exception as e:
            _audit("stripe_webhook_signature_failed", {"error": str(e)})
            raise
    else:
        _audit("stripe_webhook_unsigned",
               {"warning": "STRIPE_WEBHOOK_SECRET unset; accepting unverified payload"})
        try:
            event_payload = json.loads(body_text)
        except Exception as e:
            raise ValueError(f"invalid webhook body: {e}")

    event_type = event_payload.get("type", "")
    data = (event_payload.get("data") or {}).get("object") or {}
    result: dict[str, Any] = {
        "ok": True,
    ...
    if event_type == "checkout.session.completed":
        ...
        amount = int(data.get("amount_total") or data.get("amount_paid") or 0)
        currency = (data.get("currency") or "usd").lower()
        if currency != "usd":
            _audit("stripe_webhook_non_usd",
                   {"currency": currency, "amount": amount, "event_id": result["event_id"]})
        if amount > 0:
            new_balance = top_up(amount, source=f"stripe:{result.get('event_id') or 'unknown'}")
            result["topped_up_cents"] = amount
            result["balance_cents"] = new_balance
        else:
            result["topped_up_cents"] = 0

    elif event_type in ("charge.refunded", "checkout.session.async_payment_failed"):
        # No-op; we just record it.
        _audit("stripe_webhook_other", {"type": event_type, "event_id": result["event_id"]})
